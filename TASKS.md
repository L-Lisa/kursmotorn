...
- [ ] MG onboardas: Editorial Lugn-brand ur DB + import av C-FINAL v1–9 (`/Users/lisa/COWORK/projects/mindfulnessguiden/kursinnehall/`; parsern anpassas till veckoformatet, innehållet röres ej).
- [ ] Praxislogg: auto vid ≥90 % uppspelning + manuell (7 dagar bakåt, manuella räknas fullt — Lisas beslut); fönsterberäkningen (6×7 dagar från enrollment.starts_at) som serverfunktion/vy. **[L] MG:s meditationsljudfiler:** Lisa pekar ut/levererar dem (Storage per sektion; media_duration_sec sätts vid import) — auto-loggningen kan inte slutföras utan dem; mätmetoden loggas i DECISIONS.
- [ ] Deltagarvyn: veckovy + certstatus per krav — Editorial Lugn-ton, aldrig streak-ångest (docs/SPEC-guide-funktioner.md §3.1).
...
- [ ] FFMQ-15 pre med tidslås. **[L] Lisa pekar ut källfilen i mindfulnessguiden.se-kodbasen — kopiera items/scoring ordagrant; bygg ALDRIG egna items.** Blockerar endast FFMQ-momentet. **[L] FFMQ-användarvillkoren för kommersiell app är overifierade** (godkända specen §11.8) — flagga till Lisa före färdigbygge.
...
**GRIND 7:** sessionslogg-kriterierna (ACCEPTANCE §Fas 7) + **breathworks-regressionen körs IGEN** (tenant #1 opåverkad av MG-onboardingen).
