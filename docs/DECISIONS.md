...
- **`npm run eject -- <slug>`** → `eject-output/<slug>/` (gitignorerad): `data/*.json` (alla 23 tabeller tenant-filtrerade + `users.json` med deltagaridentiteter — GDPR-portabelt) · `brand_spec.json` · `seed/seed.sql` (genererade INSERT:ar i FK-ordning — återskapar tenantens data i en fristående Postgres/Supabase med motorns schema) · `standalone/` (fristående kursvisare) · `README.md`.
- **Standalone-mallen = statisk kursvisare**, inte en export av hela Next-appen: index + en sida per modul (markdown→HTML vid exporttillfället via `marked`, ny dev-dependency), brand-tokens inbakade som statisk CSS, minimal node-server utan beroenden. Motiv: fas 8-kriteriet är "startar lokalt och visar kursen" med "ingen tenant-context-kod kvar (statisk config)" — friköpets fulla appflytt (login/gating/prov) sker via seed.sql + motorns kodbas i den affärsförhandlingen, inte via v1-skriptet. Detta ÄR eject-snittets bevis: samma innehåll + brand renderas utan context-lagret, för att allt tenant-specifikt låg i DB.
- **Uppladdade mediafiler ingår inte** i paketet — `uploads.json` listar Storage-sökvägarna; filflytt är en driftåtgärd vid faktiskt friköp (stora filer, riktas mot köparens bucket).
- **Testartefakt-städning:** certifikat överlever numera användarradering (fas 7) ⇒ cert-/MG-sviterna raderar nu sina egna certifikatrader i after-hooken så demo-DB:n inte ackumulerar anonyma revokerade rader från testkörningar.
